...
local function create_victory_gui(player)
    ...
    local frame = player.gui.left.add {
        type = 'frame', name = 'bb_victory_gui', direction = 'vertical', caption = c .. ' won!',
    }
...
local function silo_kaboom(entity)
    ...
    surface.create_entity({
        name = 'atomic-rocket', position = center_position, force = force, source = center_position,
        target = center_position, max_range = 1, speed = 0.1,
    })
    ...
    for x = -32, 32, 1 do
        for y = -32, 32, 1 do
            ...
            if distance_to_silo < 32 and count > 0 then table.insert(drops, {p, count}) end
        end
    end
    for _, drop in pairs(drops) do
        for _ = 1, drop[2], 1 do
            entity.surface.spill_item_stack({drop[1].x + math.random(0, 9) * 0.1, drop[1].y + math.random(0, 9) * 0.1},
    ...
    for _, p in pairs(game.players) do
        if score.players[p.name] then
            ...
            table.insert(score_list, {
                name = p.name, killscore = killscore, deaths = deaths, built_entities = built_entities,
...
local function biter_killed_the_silo(event)
    ...
    if force ~= nil then return string.find(event.force.name, '_biters') end

    local cause = event.cause
    if cause ~= nil then return (cause.valid and cause.type == 'unit') end
...
local function respawn_silo(event)
    ...
    entity = surface.create_entity {
        name = entity.name, position = entity.position, surface = surface, force = force_name,
