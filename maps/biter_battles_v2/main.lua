...
local function on_player_joined_game(event)
    ...
    if player.online_time == 0 or player.force.name == 'player' then Functions.init_player(player) end
...
local function on_research_finished(event) Functions.combat_balance(event) end


local function on_console_chat(event) Functions.share_chat(event) end
...
local function on_robot_built_tile(event) Terrain.deny_bot_landfill(event) end
...
local tick_minute_functions = {
    [300 * 1] = Ai.raise_evo, [300 * 2] = Ai.destroy_inactive_biters, [300 * 3 + 30 * 0] = Ai.pre_main_attack, -- setup for main_attack
    ...
    [300 * 3 + 30 * 3] = Ai.perform_main_attack, [300 * 3 + 30 * 4] = Ai.perform_main_attack,
    [300 * 3 + 30 * 5] = Ai.perform_main_attack, [300 * 3 + 30 * 6] = Ai.perform_main_attack,
    [300 * 3 + 30 * 7] = Ai.perform_main_attack, [300 * 3 + 30 * 8] = Ai.post_main_attack,
    [300 * 4] = Ai.send_near_biters_to_silo, [300 * 5] = Ai.wake_up_sleepy_groups,
}
...
local function on_player_mined_entity(event) Terrain.minable_wrecks(event) end
...
local function on_chunk_generated(event)
    ...
    if pos.y < 0 then Terrain.generate(event) end
...
local function on_entity_cloned(event)
    ...
    if not source.valid then
        if destination.valid then destination.destroy() end
...
local function on_area_cloned(event)
    ...
    if position.y == 64 and math.abs(position.x) <= 64 then Mirror_terrain.remove_hidden_tiles(event) end
end
...
Event.add_event_filter(defines.events.on_post_entity_died, {filter = 'type', type = 'unit'})
