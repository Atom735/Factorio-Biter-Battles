...
local evo_gains = {
    ['unit-spawner'] = 0.0025, ['unit'] = 0.000025, ['turret'] = 0.001,
}

local function draw_evolution_gui()
    local seed = game.surfaces[1].map_gen_settings.seed
    local color_r = math.abs(
      simplex_noise(global.color_counter * 0.015, 0, seed)) + 0.2
    if color_r > 1 then color_r = 1 end
    local color_g = math.abs(simplex_noise(global.color_counter * 0.015, 10000,
      seed)) + 0.2
    if color_g > 1 then color_g = 1 end
    local color_b = math.abs(simplex_noise(global.color_counter * 0.015, 20000,
      seed)) + 0.2
    ...
    for _, player in pairs(game.connected_players) do
        ...
        local element = player.gui.top.add({
            type = 'sprite-button', name = 'evolution_gui',
            caption = 'Evolution: ' .. math.round(global.evolution_factor, 4)
              * 100 .. '%',
...
local function set_endgame_stats()
    ...
    game.forces.enemy.set_ammo_damage_modifier('melee', (global.evolution_factor
      - 1) * 1.5)
    game.forces.enemy.set_ammo_damage_modifier('biological',
      (global.evolution_factor - 1) * 1.5)
