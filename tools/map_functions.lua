...
f.draw_noise_tile_ring = function(surface, position, name, radius_min,
  radius_max)
    ...
    for y = radius_max * -2, radius_max * 2, 1 do
        for x = radius_max * -2, radius_max * 2, 1 do
            ...
            local noise = simplex_noise(pos.x * modifier_1, pos.y * modifier_1,
              seed)
            local distance_to_center = math.sqrt(x ^ 2 + y ^ 2)

            if distance_to_center + noise * radius_max * 0.25 < radius_max
              and distance_to_center + noise * radius_min * 0.25 > radius_min then
...
f.draw_noise_entity_ring = function(surface, position, name, force, radius_min,
  radius_max)
    ...
    for y = radius_max * -2, radius_max * 2, 1 do
        for x = radius_max * -2, radius_max * 2, 1 do
            local pos = {x = x + position.x, y = y + position.y}
            local noise = simplex_noise(pos.x * modifier_1, pos.y * modifier_1,
              seed) + simplex_noise(pos.x * modifier_2, pos.y * modifier_2, seed)
                            * 0.2
            local distance_to_center = math.sqrt(x ^ 2 + y ^ 2)
              ...
              and distance_to_center + noise * radius_min * 0.25 > radius_min then
                if surface.can_place_entity({name = name, position = pos}) then
                    surface.create_entity({
                        name = name, position = pos, force = force,
                    })
                end
            end
        end
...
f.draw_rainbow_patch = function(position, surface, radius, richness)
    ...
    for y = radius * -3, radius * 3, 1 do
        for x = radius * -3, radius * 3, 1 do
            local pos = {x = x + position.x, y = y + position.y}
            local noise = simplex_noise(pos.x * modifier_2, pos.y * modifier_2,
              seed) + simplex_noise(pos.x * modifier_3, pos.y * modifier_3, seed)
                            * modifier_4
            local distance_to_center = math.sqrt(x ^ 2 + y ^ 2)
            ...
            if amount > 1 then
                if surface.can_place_entity({
                    name = ore, position = pos, amount = amount,
                }) then
...
f.draw_entity_circle = function(position, name, surface, radius,
  check_collision, amount)
    if not position then return end
            ...
            if distance_to_center <= radius then
                if check_collision then
                    if surface.can_place_entity({name = name, position = pos}) then
                        if amount then
                            surface.create_entity({
                                name = name, position = pos, amount = amount,
                            })
                        else
...
f.draw_smoothed_out_ore_circle = function(position, name, surface, radius,
  richness)
    if not position then return end
    if not name then return end
    if not surface then return end
...
f.draw_crazy_smoothed_out_ore_circle = function(position, name, surface, radius,
  richness)
    if not position then return end
    if not name then return end
    if not surface then return end
...
f.create_cluster = function(name, pos, size, surface, spread, resource_amount)
    ...
    for z = 1, size, 1 do
        ...
        local modifier_raffle = {
            {0, y * -1}, {y * -1, 0}, {y, 0}, {0, y}, {y * -1, y * -1}, {y, y},
            {y, y * -1}, {y * -1, y},
        }
        ...
        for x = 1, 8, 1 do
            ...
            if resource_amount then
                if surface.can_place_entity({
                    name = name, position = pos, amount = resource_amount,
                }) then
                    surface.create_entity {
                        name = name, position = pos, amount = resource_amount,
                    }
